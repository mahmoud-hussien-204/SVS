...
import {apiGetFAQList} from "../services";
...
const FAQList = () => {
  ...
  const {data, isLoading} = useQuery({
    queryFn: () => apiGetFAQList({page, limit, search}),
    queryKey: ["admin-get-faq-list", page, limit, search],
  });

  const totalPages = data?.recordsTotal ? Math.ceil(data.recordsTotal / limit) : 1;

  const faqData = useMemo(() => data?.data || [], [data]);
  ...
  return (
    <Box>
      <TableBoxedLayoutContainer>
        <TableBoxedLayoutTHead>
          <TableBoxedLayoutTR>
            <TableBoxedLayoutTH>Question</TableBoxedLayoutTH>
          ...
          {isLoading ? (
            ...
            faqData.map((item) => (
              <TableBoxedLayoutTR key={item.id}>
                <TableBoxedLayoutTD>{item.question}</TableBoxedLayoutTD>
                <TableBoxedLayoutTD>
